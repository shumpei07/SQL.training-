...
SELECT name, population FROM countries WHERE population >=100000;

-- 問5
-- 平均寿命が56歳から76歳の国をすべて抽出してください。
SELECT name, life_expectancy FROM countries WHERE life_expectancy >= 56 AND life_expectancy <= 76;

-- 問6
-- 国コードがNLB,ALB,DZAのもの市区町村をすべて抽出してください。
SELECT name, country_code FROM cities WHERE country_code = 'NLB' OR country_code = "ALB" OR country_code = "DZA";

-- 問7
-- 独立独立記念日がない国をすべて抽出してください。
SELECT name, indep_year FROM countries WHERE indep_year  IS NULL;

-- 問8
-- 独立独立記念日がある国をすべて抽出してください。
SELECT name, indep_year FROM countries WHERE indep_year IS NOT NULL;
...
SELECT name, code, indep_year FROM countries WHERE (code = 'DZA' OR code = 'ALB') AND indep_year < 1990;
...
SELECT countries.name as 国名,cities.name as 市区町村名, countrylanguages.language FROM countries JOIN cities ON countries.code = cities.country_code JOIN countrylanguages ON countries.code = countrylanguages.country_code;
...
FROM celebrities LEFT JOIN countries ON celebrities.country_code = countries.code LEFT JOIN countrylanguages ON countries.code = countrylanguages.country_code WHERE (countrylanguages.country_code, countrylanguages.percentage) IN ( SELECT country_code, MAX(percentage) FROM countrylanguages GROUP BY country_code );

-- 問29
-- 全ての有名人の名前と国名を出力してください。 ただしテーブル結合せずサブクエリを使用してください。
SELECT name, (SELECT countries.name FROM countries WHERE celebrities.country_code = countries.code) AS 国名 FROM celebrities;

-- 問30
-- 最年長が50歳以上かつ最年少が30歳以下の国を表示させてください。
SELECT country_code, MAX(age) as 'MAX(ce.age)', MIN(age) as 'MIN(ce.age)' FROM celebrities WHERE country_code != '' GROUP BY country_code HAVING MAX(age) >= 50 AND MIN(age) <= 30;

-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。
SELECT '1991' AS 誕生年, COUNT(*) AS 人数 FROM celebrities WHERE YEAR(birth) = 1991 UNION SELECT '1981' AS 誕生年, COUNT(*) AS 人数 FROM celebrities WHERE YEAR(birth) = 1981;
